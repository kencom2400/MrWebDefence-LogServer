...
**問題点**:
- 同じセクション内で異なる例を使用している
- 読者が混乱する可能性がある
- ドキュメントの整合性が損なわれる

**✅ 良い例**: ドキュメント全体で同じ例を使用する
...
**問題点**:
- MySQLでは多くのDDL文が暗黙的なコミットを引き起こすため、トランザクション内でDDLを実行できないのはMySQLの基本的な動作
- 設定で変更できるものではない
- 他のDB（PostgreSQL等）との違いを明確にしていない
...
**問題点**:
- 「ロールバック」という言葉がFlyway有料版の`undo`機能と混同される可能性がある
- 実際には変更を取り消すための新しいマイグレーションを作成する「ロールフォワード」と呼ばれるアプローチ
- 誤解を招く可能性がある
...
**問題点**:
- 実行方法は「アプリケーション起動時」だが、確認方法は「CI/CDパイプライン」
- Flywayの`migrate`コマンドは差分（未適用分）のみを適用するため、「全マイグレーションを必ず実施」という目的を達成できない
- 整合性が取れていない

**✅ 良い例**: 実行方法と確認方法の整合性を保つ
...
**理由**:
- 実行方法と確認方法の整合性が保たれる
- 目的（全マイグレーションを必ず実施）を達成できる
- 実装時の混乱を防ぐことができる

### 実装チェックリスト

- [ ] ドキュメント全体で同じ例を使用しているか確認
- [ ] 技術的な動作を正確に記述しているか確認
- [ ] 用語を明確に定義しているか確認
- [ ] 実行方法と確認方法の整合性を確認
- [ ] 目的を達成できる実行方法になっているか確認

**参照**: PR #39 - Issue MWD-79: Task 2.3: Flywayマイグレーション設計（Gemini Code Assistレビュー指摘）

### ツールのデフォルト動作の理解

**❌ 悪い例**: ツールのデフォルト動作を理解せずにディレクトリ構成を設計する

```markdown
src/main/resources/db/migration/          # 通常のマイグレーション
src/main/resources/db/migration/seed/     # 初期データ投入用
```

**問題点**:
- Flywayのデフォルト動作（サブディレクトリの再帰的なスキャン）を理解していない
- `locations=db/migration`と指定した場合でも、サブディレクトリ内のファイルが実行対象に含まれてしまう
- 意図せず初期データ投入が実行される可能性がある

**✅ 良い例**: ツールのデフォルト動作を理解してディレクトリ構成を設計する
...
**理由**:
- ツールのデフォルト動作を理解して設計できる
- 意図しない実行を防ぐことができる
- `locations`パラメータで明確に使い分けることができる

### コマンド例の統一性確保

**❌ 悪い例**: ドキュメント内で異なる方式のコマンド例を使用する
...
**問題点**:
- 「クラスパス上に配置する」と記載されているのに、コマンド例では`filesystem:`を使用している
- 矛盾しており、開発者に混乱を招く
- Maven/Gradleプロジェクトの標準的な構成と異なる

**✅ 良い例**: ドキュメント全体で統一された方式を使用する
...
- [ ] 使用するツールのデフォルト動作を理解しているか確認
- [ ] ディレクトリ構成がツールのデフォルト動作と整合しているか確認
- [ ] ドキュメント全体でコマンド例の方式が統一されているか確認
- [ ] 標準的な構成（Maven/Gradle等）に合わせているか確認

**参照**: PR #39 - Issue MWD-79: Task 2.3: Flywayマイグレーション設計（Gemini Code Assistレビュー指摘 - 第2回）

### シェルスクリプトでのパスワード管理

**❌ 悪い例**: パスワードをコマンドライン引数で直接渡す

```bash
mysql -h"${DB_HOST}" -u"${DB_USER}" -p"${DB_PASSWORD}" -e "SELECT 1;"
flyway migrate -user="${DB_USER}" -password="${DB_PASSWORD}"
```

**問題点**:
- パスワードがコマンドライン引数として渡されると、システムのプロセスリスト（`ps`コマンド等）からパスワードが見えてしまう
- セキュリティ上のリスクがある
- ログファイルにパスワードが記録される可能性がある
...
flyway migrate -user="${DB_USER}"
```

**理由**:
- パスワードがプロセスリストに表示されない
- セキュリティリスクが低減される
- ログファイルにパスワードが記録されない

**注意事項**:
- `MYSQL_PWD`環境変数はMySQLクライアントが自動的に読み込む
- `FLYWAY_PASSWORD`環境変数はFlyway CLIが自動的に読み込む
- 環境変数は子プロセスにも継承されるため、スクリプト内でのみ使用する場合は注意が必要
...
```bash
local charset=$(mysql -h"${DB_HOST}" -u"${DB_USER}" "${DB_NAME}" -N -e "SELECT @@character_set_database;")
local collation=$(mysql -h"${DB_HOST}" -u"${DB_USER}" "${DB_NAME}" -N -e "SELECT @@collation_database;")
```
...
**✅ 良い例**: 1回のクエリで必要な情報を取得する

```bash
local charset
local collation
read -r charset collation <<< "$(mysql -h"${DB_HOST}" -u"${DB_USER}" "${DB_NAME}" -N -e "SELECT @@character_set_database, @@collation_database;")"
```
...
**問題点**:
- ユーザーがコマンドライン引数でデフォルト値を上書きした場合でも、ログには初期値が表示される
- 混乱を招く可能性がある
- デバッグが困難になる
...
- [ ] パスワードをコマンドライン引数で直接渡していないか確認
- [ ] 環境変数経由でパスワードを渡しているか確認
- [ ] 同じコマンドを複数回実行していないか確認（1回のクエリで必要な情報を取得できるか検討）
- [ ] 引数解析を最初に実行しているか確認
- [ ] ログに正しいパラメータ値が表示されるか確認

**参照**: PR #40 - Issue MWD-80: Task 2.4: データベース初期化スクリプト実装（Gemini Code Assistレビュー指摘）

### シェルスクリプトの引数解析の堅牢化

**❌ 悪い例**: 引数チェックなしでオプションの引数を取得する
...
**問題点**:
- 引数を必要とするオプション（例: `-h`）が引数なしで渡された場合、次に続く別のオプション（例: `-P`）を引数として解釈してしまう
- 例: `./script.sh -h -P 3307` と実行すると、`DB_HOST` に `-P` が設定されてしまう
- 予期せぬ動作につながる可能性がある

**✅ 良い例**: 引数の存在と妥当性をチェックする
...
**実装チェックリスト**:
- [ ] 引数を必要とするオプションで、引数の存在をチェックしているか
- [ ] 引数が別のオプション（`-`で始まる）でないかチェックしているか
- [ ] エラーメッセージが分かりやすいか
- [ ] ヘルプメッセージを表示してから終了しているか

**参照**: PR #40 - Issue MWD-80: Task 2.4: データベース初期化スクリプト実装（Gemini Code Assistレビュー指摘 - 第2回）
