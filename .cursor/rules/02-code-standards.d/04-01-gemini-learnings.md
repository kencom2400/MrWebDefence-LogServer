...
### モニタリングエンドポイントのセキュリティ（MrWebDefence-LogServer PR #27）

**学習元**: PR #27 - MWD-53: Fluentdベースのログサーバー実装（Geminiレビュー指摘）

#### 内部モニタリングエンドポイントの公開制御

**問題**: モニタリングエンドポイント（monitor_agent, Prometheus）が`0.0.0.0`にバインドされ、ネットワークに公開されている

**背景**:
- 内部モニタリングエンドポイントは診断情報やメトリクスを公開する
- 悪意のある者にシステムの内部状態を知られるリスクがある
- デフォルトでは最も安全な設定（ローカルホストのみ）にすべき

**解決策**: 環境変数でバインドアドレスを制御可能にし、デフォルトを`127.0.0.1`に設定
...
<!-- ✅ 良い例: 環境変数で制御、デフォルトはローカルのみ -->
<source>
  @type monitor_agent
  bind "#{ENV['MONITORING_BIND_ADDR'] || '127.0.0.1'}"
  port 24220
</source>
```
...
```bash
# デフォルト（最も安全）- ローカルホストのみ
MONITORING_BIND_ADDR=127.0.0.1

# 特定のサブネットIPアドレス（推奨）
MONITORING_BIND_ADDR=10.0.1.100

# 全インターフェース公開（ファイアウォールで制御必須）
...
1. **デフォルトは最も安全な設定にする**
   - `127.0.0.1`（ローカルのみ）をデフォルトにする
   - 明示的に設定しない限り外部公開しない

2. **環境変数でカスタマイズ可能にする**
   - 本番環境のネットワーク構成に合わせて柔軟に設定できるようにする
   - 特定のサブネットIPを指定できるようにする

3. **ドキュメントで明示する**
   - デフォルト設定が何か明記する
   - 各設定値のセキュリティへの影響を説明する
   - 推奨設定を提示する

4. **テスト環境では公開する**
   - テスト環境では`0.0.0.0`で公開し、外部からアクセスできるようにする
   - 本番とテストで異なる設定を使い分ける

**適用対象**:
- モニタリングエンドポイント（Fluentd monitor_agent, Prometheus等）
- 管理用エンドポイント
- デバッグ用エンドポイント
- 内部診断用API
...
**理由**:
- セキュリティのデフォルトは「最も制限的」であるべき
- 必要に応じて緩和する方が、デフォルトで開放するより安全
- ネットワーク構成に応じた柔軟な設定が可能
- インシデントのリスクを低減できる

**参照**: PR #27 - MWD-53: Fluentdベースのログサーバー実装（Geminiレビュー指摘）
