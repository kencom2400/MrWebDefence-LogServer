...
class TestLogBuffer:
    ...
    async def test_add_batch_normal(self):
        ...
        buffer = LogBuffer(
            max_count=10, max_memory_mb=1, flush_interval_sec=60, storage=mock_storage
        )

        log_entries = [
            LogEntry(
                timestamp=datetime.now(timezone.utc), level="info", message=f"Test message {i}"
            )
            for i in range(5)
        ]
    ...
    async def test_buffer_full_error(self):
        ...
        more_logs = [
            LogEntry(timestamp=datetime.now(timezone.utc), level="info", message="One more log")
        ]
